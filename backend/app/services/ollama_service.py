...
async def send_generate_request(model: str, prompt: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout_duration) as client:
            ...
            logger.info(f"✅ Model '{model}' successfully responded")
...
async def send_message(model: str, messages: List[Dict[str, str]]) -> str:
    ...
    try:
        ...
        result = await send_generate_request(model, prompt)
        logger.info(f"🎉 Model '{model}' successfully completed /api/generate request")
        return result
...
async def send_streaming_message(model: str, messages: List[Dict[str, str]]) -> str:
    ...
    print(f"🌊 OLLAMA: Отправка сообщения с использованием потокового режима к модели '{model}'")
    logger.info(f"🌊 Starting streaming request to model: {model}")


#     instruction = (
#     f"""
#     Ты — спортивный аналитик по настольному теннису.

#     Твоя задача — извлечь ключевую информацию и выдать результат строго в формате JSON со следующими полями:

#     {{
#     "player_1": "Имя первого игрока",
#     "player_2": "Имя второго игрока",
#     "score": "Счёт по партиям (например: 11-9, 9-11, 11-7)",
#     "league": "Название лиги (например: Любители)",
#     "analysis": "Аналитический разбор: кто был сильнее, где были триггеры, почему победил или проиграл"
#     }}

#     Отвечай строго на русском языке. Возвращай только JSON, без пояснений.

    
#     """
# )
#     if messages and messages[0]["role"] == "user":
#         messages[0]["content"] = instruction + "\n\n" + "Вот сообщение: " + messages[0]["content"]
#     else:
#         messages = [{
#             "role": "user",
...
#         }] + messages
    ...
    try:
        ...
        # Читаем ответ построчно
            ...
            logger.info(f"✅ Streaming model '{model}' completed response with {len(full_response)} characters")
...
async def query_ollama(prompt: str, model: str) -> str:
    ...
    try:
        logger.info(f"🔍 Проверка доступности модели {model} перед запросом  Aidana call")
        ...
        async with httpx.AsyncClient(timeout=600) as client:
            ...
            logger.info(f"✅ Direct query to model '{model}' successful")
            return data["response"]
    except Exception as error:
...
def trigger(data: dict) -> str:
    """
    Анализирует матч и формирует промпт для Ollama на основе выявленных триггеров.
    """
    игрок_1 = data.get("игрок_1")
    игрок_2 = data.get("игрок_2")
    счёт = data.get("счёт", "")
    этап = data.get("этап", "")
    турнир = data.get("турнир", "")
    лига = data.get("лига", "")

    # 1. Парсим общий счёт (например, "2-3")
    try:
        match_result = счёт.split("(")[0].strip()
        won_sets_1, won_sets_2 = map(int, match_result.split("-"))
    except:
        won_sets_1 = won_sets_2 = 0

    # 2. Извлекаем отдельные сеты
    try:
        set_scores_raw = re.search(r"\((.*?)\)", счёт)
        raw_sets = set_scores_raw.group(1).strip().replace(",", "").split()
        set_scores = []
        for set_str in raw_sets:
            try:
                p1, p2 = map(int, set_str.split("-"))
    ...
    результат = f"{won_sets_1}-{won_sets_2}"
    print(f"Результат матча: {результат} (сеты: {set_scores}) 1: {won_sets_1}, 2: {won_sets_2}")

    # 3. Формируем список наблюдений (триггеров)
    observations = []

    # 🔹 Триггер: выиграл 1 сет и проиграл
    if won_sets_1 == 1 and won_sets_2 > won_sets_1:
        observations.append(
            f"{игрок_1} выиграл только один сет, а затем проиграл матч. "
            f"Это может указывать на нестабильность или психологическую/физическую невыносливость."
        )
    if won_sets_2 == 1 and won_sets_1 > won_sets_2:
        observations.append(
            f"{игрок_2} выиграл только один сет, а затем проиграл матч. "
            f"Это может указывать на нестабильность или психологическую/физическую невыносливость."
        )

    # Здесь будут другие триггеры в будущем...

    # 4. Формируем промпт
    context = (
        f"Матч между {игрок_1} и {игрок_2} в рамках турнира {турнир} ({этап}, лига {лига}).\n"
        f"Итоговый счёт: {счёт} (сеты {результат}).\n"
    )

    context += (
        f"""
Ты — спортивный аналитик по настольному теннису, который делает глубокий анализ матчей и помогает выявить слабые и сильные стороны игроков. Сделай максимально глубокий анализ матча, найди слабые и сильные стороны игроков, обрати внимание на лигу, на рейтинг игроков. Учитывая указанные наблюдения ниже какие выводы можно сделать о форме, психологии или стратегии игроков?
